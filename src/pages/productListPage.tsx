  ...
  return (
    <>
      ...
      <div className='prodlist-content'>{product.map((prod: {id: number; nome: string; preco: string}) => (
          <div className='prodlist'><Link className='prod-list'key={prod.id} to={`/categorias/${id.id}/${prod.id}`}>
          <div>
        ...
        <div className='preco'>
            <h2>R$ {prod.preco}</h2>
            <FontAwesomeIcon icon={faShoppingCart} onClick={() => cartHandler()} cursor='pointer' />
            <p>{productCart}</p>
          </div></div>
        ))}</div>
      </>
